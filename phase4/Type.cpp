...
Type Type::promote() const{
    //std::cout<<"in promote "<<*this<<endl;
    if(*this == Type(CHAR,0)){
        //std::cout<<"return char"<<endl;
        return Type(INT, 0);
    }
    if(_kind == ARRAY){
...
bool Type::isValue() const{
    Type t = promote();
    if(t._kind == SCALAR && t._indirection > 0){
        return true;
    }
    if( t == Type(INT,0)){
        return true;
...
bool Type::isCompatible(const Type &t) const{
    Type l1 = this->promote();
    //cout<<"what the fuck l1 "<<l1<<endl;
    Type r1 = t.promote();
    //std::cout<<"compatible check: "<<l1<<" and "<<r1<<std::endl;
    if(l1.isValue() && r1.isValue())
        if(l1.indirection() == r1.indirection() && l1.specifier() == r1.specifier())
            return true;
    if(l1.isScalar() && l1._indirection > 0)
        if(r1.isScalar() && r1.indirection() == 1 && r1.specifier() == VOID)
            return true;
    if(r1.isScalar() && r1._indirection > 0)
        if(l1.isScalar() && l1.indirection() == 1 && l1.specifier() == VOID)
            return true;
    return false;
}
