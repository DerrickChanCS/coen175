...
unsigned Type::size() const{
    unsigned bytes;
    if(isArray()){ //if t is an array
        if(indirection() > 0){ //if t is an array of pointers
            bytes = 4 * length();
        }else if(specifier() == CHAR){ //if t is an array of chars
            //cout<<"array of char: "<<t.length()<<endl;
            bytes = length();
        }else if(specifier() == INT){ //if t is an array of ints
            bytes = 4 * length();
        }
    }else if(isPointer()){ //if t is a pointer
        bytes = 4; 
