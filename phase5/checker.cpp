...
Scope *closeScope()
{
    ...
    if(old == outermost){
        cout<<".data"<<endl;
        for(int i = 0; i< (int)old->symbols().size(); i++){
            if(!old->symbols()[i]->type().isFunction()){
                cout<<".comm\t"<<old->symbols()[i]->name();
                cout<<", "<<old->symbols()[i]->type().size();
                cout<<endl;
            }
        }
        cout<<endl;
